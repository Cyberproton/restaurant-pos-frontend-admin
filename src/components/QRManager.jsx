...
export default class QRManager extends Component {
    constructor(props) {
        ...
        this.toggleDeleteQrCodeConfirmation = this.toggleDeleteQrCodeConfirmation.bind(this)
    }
    ...
    render() {
        ...
        const addForm = this.state.isShowingAddQrCodeForm ? 
            ...
            /> : 
            null
        const updateForm = this.state.isShowingUpdateQrCodeForm && qrcode ? 
            <UpdateQrCodeForm
                qrcode={qrcode}
                handleInputChange={this.handleInputChange} 
            ...
            /> : 
            null
        const deleteConfirmation = this.state.isShowingDeleteQrCodeConfirmation && qrcode ? 
            <DeleteQrCodeConfirmation qrcode={qrcode} show={true} handleSubmit={this.handleDeleteQrCode} handleToggle={this.toggleDeleteQrCodeConfirmation}/> :
            null
        return (
            <Container className="mt-3">
                ...
                <QrCodeList qrcodes={this.state.qrCodes} handleClick={this.onQrCodeClick} selectedQrCode={qrcode}/>
                <Container>
                    <Row>
                        <Col>
                        ...
                        <Col>
                            <Button onClick={this.toggleUpdateQrCodeForm} disabled={!qrcode}>Sửa mã</Button> 
                        </Col>
                        <Col>
                            <Button 
    ...
    handleUpdateQrCode(event) {
        ...
        const qrcode = this.state.qrCodeForm
        const id = this.state.selectedQrCode ? this.state.selectedQrCode._id : undefined
    ...
    toggleDeleteQrCodeConfirmation() {
        this.setState(prev => ({
            isShowingAddQrCodeForm: false,
            isShowingUpdateQrCodeForm: false,
            isShowingDeleteQrCodeConfirmation: !prev.isShowingDeleteQrCodeConfirmation,
        }))
    }
}
...
function QrCodeList(props) {
    ...
    if (qrcodes && qrcodes.length > 0) {
        const selectedId = props.selectedQrCode ? props.selectedQrCode._id : undefined
        const list = qrcodes.map((value) => <QrCodeRow key={value._id} qrcode={value} handleClick={props.handleClick} selected={value._id === selectedId}/>)
        return (
...
function QrCodeRow(props) {
    ...
    return (
        <tr 
            ...
            onClick={() => props.handleClick(qrcode._id)}
        >
    ...
    return (
        <div className="container border border-primary">
            <Container className="my-4">
                <h5><FaPlus/> Thêm mã QR</h5>
                <Form onSubmit={(e) => { props.handleAddQrCode(e); props.toggleAddQrCodeForm() }}>
        ...
        <div className="container border border-primary">
            <Container className="my-4">
                ...
                <Form>
                    ...
                    <Button variant="primary" onClick={(e) => { props.handleSubmit(e); props.handleToggle() }}>
                        Cập nhật mã QR
                    </Button>
                </Form>
            </Container>
...
function DeleteQrCodeConfirmation(props) {
    ...
    return (
        ...
        >
            ...
            <Button variant="danger" onClick={(e) => { props.handleSubmit(e); props.handleToggle() } }>
                Xóa mã QR
            </Button>
            </Modal.Footer>
        </Modal>
