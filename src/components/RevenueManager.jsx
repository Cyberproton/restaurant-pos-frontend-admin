...
import { Button, Container, Dropdown, DropdownButton, Form, Modal, Table, Row, Col } from "react-bootstrap";
...
export default class RevenueManager extends Component {
    ...
    render() {
        ...
        const filteredBills = this.state.bills.filter(bill => new Date(bill.date) >= dateFrom && new Date(bill.date) < dateToNext);
        const revenue = filteredBills.map(bill => bill.payment).reduce((total, payment) => total + payment, 0);
        ...
        const keys = Object.keys(revenuePerFormat).map(k => new Date(k)).sort((a, b) => a - b);
        console.log(keys);
        for (const p of keys) {
        ...
        return (
            <Container className="margin-side pt-1">
                <Container className="mt-5">
                    <Row>
                        ...
                        <Col className="d-flex align-items-end">
                            <DropdownButton id="dropdown-item-button" title={"Dữ liệu: " + formatName} variant="outline-dark">
                                <Dropdown.Item 
                                    as={Button} 
                ...
                <Container className="d-flex mt-5 justify-content-around">
                    <Container>
                        <h5><FaCommentDollar/>  Tổng chi: N/A</h5>
                    </Container>
                    <Container>
                        <h5><FaCoins/>  Tổng thu: {revenue}</h5>
                    </Container>
                    <Container>
                        <h5><FaDollarSign/>  Lợi nhuận: N/A</h5>
                    </Container>
                </Container>
